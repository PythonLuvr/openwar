...
OpenWar is versioned. Current: v0.11.1 (framework doc + runtime + multi-agent orchestration + cli-bridge adapter powered by published `@pythonluvr/squire@1.0.0` + per-role adapter mixing + persistent project memory + MCP-server-mode for cli-bridge + structured trace event stream with focused inspect modes, replay, and local dashboard + descriptive history analytics via `openwar history` + adaptive autonomy plumbing via `openwar learn` and `learned_profile:` frontmatter with conservative thresholds + conversation-driven entry point via `openwar chat` with structured tool-call intent extraction + mid-tool-call cancellation across every native tool with `tool_cancelled` trace event). Drop-in upgrades preserve compatibility within a major version; major bumps may rename phases or change the brief format. The runtime package matches the framework doc's version one-for-one.
